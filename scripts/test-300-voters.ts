#!/usr/bin/env npx tsx
/**
 * Test script: 300 Voters Simulation (E2E Stress Test)
 *
 * Simulates a complete election with 300 voters:
 * 1. Create election with threshold key ceremony
 * 2. Register trustees and complete ceremony
 * 3. Register 300 voters
 * 4. Cast 300 votes (encrypted via VeilForms)
...
async function main() {
  console.log('╔════════════════════════════════════════════════════════════╗');
  console.log('║       TVS - 300 Voters E2E Stress Test                     ║');
  console.log('╚════════════════════════════════════════════════════════════╝\n');
  ...
  const electionData = await api('/api/elections', {
    ...
    body: JSON.stringify({
      ...
      description: 'Stress test with 300 voters',
  ...
  // ==========================================================================
  // Step 6: Cast 300 Votes
  // ==========================================================================
  console.log(`🗳️  Step 6: Casting ${VOTER_COUNT} votes...`);

  // Distribute votes (5 candidates)
  // Alice: 90, Bob: 75, Carol: 60, David: 45, Eve: 30
  const voteDistribution = [
    ...Array(90).fill(0),  // Alice
    ...Array(75).fill(1),  // Bob
    ...Array(60).fill(2),  // Carol
    ...Array(45).fill(3),  // David
    ...Array(30).fill(4),  // Eve
  ];
  ...
  console.log('✅ 300 voter E2E test completed successfully!\n');
}
